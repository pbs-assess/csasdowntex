...
***Compile often*** to avoid a seemingly insurmountable number of errors after cutting and pasting many chunks. Compiling a section once every chunk or two is a good idea.
...
# Note there is no `french` toggle argument in this function, it is unnecessary
plot_example <- function(){

  # Use the fr() function to determine which language to use
  ...
  # To determine if a word's translation exists in the `rosettafish` package, simply run the
  # line below with the word substituted in and it will either give you the translation or
  # tell you it is not in the database.
  area_col_nm <- rosettafish::en2fr("Area")
  area_col <- rlang::sym(area_col_nm)

  # Rename data frame columns with either English or French terms for display on the plot legend
  d <- midwest |> 
...
The following are an English and French version of a table
