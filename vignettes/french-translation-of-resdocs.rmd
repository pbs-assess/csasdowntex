...
1. Send the English version in for translation to government translators. Once it comes back, create a second document directory containing a complete copy of the document source, then modify the English paragraphs to be French based on what is written in the translated document and change the Figures and tables into French versions.
...
1. Creating a second document directory with an entirely separate French document is prone to error when something changes in the document (a plot or table changes). The authors have to remember to change the code to create that plot/table in both the English version and the French version in exactly the same way. This will lead to two documents which are not quite the same because of human error. The question may then be asked, "which plot/tables from which document are actually correct?". Theoretically, one should not create the French document folder until the English one has been finalized, but in reality some changes end up happening after that time has passed.
...
You can now paste the French text exactly as it is translated into the source files (**\*.rmd** and **\*.r** files). You do not need to and should not use escape sequences for special French characters. If you are doing so, or using escape sequences for any other special characters, you will encounter all sorts of errors as you progress.
...
***Compile often*** to avoid a seemingly insurmountable number of errors after cutting and pasting many chunks. Compiling each section or **\*.Rmd** file once every chunk or two is a good idea.
...
plot_example <- function(){
  ...
  # The csasdown::tr()` function is a wrapper for `rosettafish::en2fr()`
  area_col_nm <- tr("Area")
  ...
  # along with the walrus operator (:=)
  g <- ggplot(d, aes(x = !!area_col, y = poptotal)) + 
    ...
    theme_classic() +
    # Use the fr() function to determine which language to use to make the plot
    labs(title = ifelse(fr(),
...
```{r plot-example-fr, echo = TRUE}
# French
options(french = TRUE)
plot_example()

# Reset the french option back to English
options(french = FALSE)
```
...
table_example <- function(){
  ...
  model <- ifelse(fr(),
                  ...
                  "Vehicle model")
  mpg <- ifelse(fr(),
                "Miles par gallon", # These are the same
                "Miles per gallon") #  in bath languages
  cyl <- ifelse(fr(),
                "Nombre de cylindres",
                "Number of cylinders")
  hp <-  ifelse(fr(),
  ...
  d <- mtcars |> 
    ...
    dplyr::select(1, mpg, cyl, hp, gear) |> 
  ...
  # Remove all rows except those starting with "Merc"
  d <- d |> 
...
Notice in the French table that decimal points in values are automatically set to the correct marks (periods for English and commas for French). This is done in the internals of `csasdown`.
