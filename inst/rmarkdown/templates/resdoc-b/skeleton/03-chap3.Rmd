...
## TABLES
By far the easiest way to present tables in your report is to store the contents of the table in a CSV or Excel file, then read that file in to your R Markdown document as a data frame. Then you can style the table with the `kable` function, or functions in the [kableExtra](https://cran.r-project.org/web/packages/kableExtra/index.html) pacakge. We recommend using `csasdowntex::csas_table()`, which simply passes some necessary arguments to `knitr::kable()` so that the tables will render correctly for CSAS.
...
## TABLEAUX
La façon la plus simple de présenter des tableaux dans votre rapport est de stocker le contenu du tableau dans un fichier CSV ou Excel, puis de lire ce fichier dans votre document R Markdown comme un cadre de données. Vous pouvez ensuite styliser le tableau avec la fonction `kable` ou les fonctions du paquet [kableExtra](https://cran.r-project.org/web/packages/kableExtra/index.html). Nous recommandons d'utiliser `csasdowntex::csas_table()`, qui passe simplement quelques arguments nécessaires à `knitr::kable()` afin que les tableaux soient rendus correctement pour le SCCS.
...
csasdowntex::csas_table(widelong_df,
