...
To render this document, run this code: `csasdowntex::render()`. This will produce a PDF. To see what the output of the preprocessor looks like (what bookdown actually uses to create the document) compile the document like this: `csasdowntex::render(keep_files = TRUE)`. This will leave modified files behind, all of which start with *tmp-* and correspond to the files you have included in your project.
...
`csasdowntex::render(keep_files = TRUE)`. If you look at *tmp-01-chap1.Rmd* you'll see that in this chunk (*chap01-para-7-en*) the text inside `cat()` has been modified to insert the R code into the string to hold the output as seen above. For further explanation, see Section \@ref(rendering).")
```
...
To check if you are missing any YAML entries, run `csasdowntex::check_yaml()`. It will also be run automatically when you run `csasdowntex::render()` and you will get an error message if you are missing any.
")
```
