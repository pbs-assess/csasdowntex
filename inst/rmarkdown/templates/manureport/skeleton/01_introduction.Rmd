...
cat("Welcome to report generation using `csasdowntex`. Included in this example document are many useful bits of information on how to write a reproducible report document using  the `csasdowntex` package, which is based on [Rmarkdown](https://rmarkdown.rstudio.com/) and [Bookdown](https://bookdown.org/). The information given in this example document should be used with all three `csasdowntex` document types: Research Document (`resdoc`), Science Response (`sr`), and Technical Report (`techreport`).")
```

```{r para-chap01-1-fr, eval=fr(), results = 'asis', needs_trans = FALSE}
cat("(This is where you would place the French translation of the paragraph. Once you do, set needs_trans to FALSE in the chunk header) Bienvenue dans la génération de rapports à l'aide de `csasdowntex`. Ce document d'exemple contient de nombreuses informations utiles sur la façon d'écrire un rapport reproductible à l'aide du paquetage `csasdowntex`, qui est basé sur [Rmarkdown] (https://rmarkdown.rstudio.com/) et [Bookdown] (https://bookdown.org/). Les informations données dans cet exemple doivent être utilisées avec les trois types de documents `csasdowntex` : Document de recherche (`resdoc`), Réponse scientifique (`sr`), et Rapport technique (`techreport`).
...
cat("
If you get stuck on anything, read the `csasdowntex` [README](https://github.com/pbs-assess/csasdowntex/blob/master/README.md) carefully and all the [Wiki pages](https://github.com/pbs-assess/csasdowntex/wiki). There are many tips and tricks located in those pages. 
...
Here is an example equation with the code used to generate it. Note that `csasdowntex` automatically numbers it on the right-hand side of the page. It does this consecutively throughout the document sections, but appendices are each numbered on their own, e.g. A.1, A.2, ... for Appendix A, and B.1, B.2, ... for Appendix B.")
```
...
csasdowntex::csas_table(refs_tab,
  align = c("r", "r"),
  caption = ifelse(fr(),
                   "French goes here",
