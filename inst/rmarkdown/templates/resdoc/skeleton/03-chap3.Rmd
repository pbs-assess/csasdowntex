...
widelong_df <- rnorm(length(yrs) * length(ages), 20) |> 
  round(4) |> 
  matrix(nrow = length(yrs),
         ncol = length(ages),
...
```{r example-fig, fig.cap = "(ref:example-fig-en)"}

plot(1:10)
```

Here is a reference to the figure: Figure \@ref(fig:example-fig).  Note the use of the `fig:` code here.  By naming the R chunk that contains the figure, we can then reference that figure later as done in the first sentence here. We can also specify the caption for the figure via the R chunk option `fig.cap`. Note that there are two reference lines above the knitr chunk with the names `ref:example-fig-en` and `ref:example-fig-en-alt`, and that `fig.cap` is set to use what is written in the `ref:example-fig-en` reference. This syntax allows for a nicer view of the caption, especially longer captions and allows for the use of embedded inline R chunks inside the text. The editor will also show this text with Markup syntax highlighting, which is not available inside the knitr chunk options brackets.

The alt text reference, `ref:example-fig-en-alt`, can also contain inline R code in the format `` `r # R code here` ``. The alt text **cannot contain commas** due to limitations of the `tagpdf` \LaTeX\ package.
...
(ref:delaysboxplot-en-alt) This plot shows that American Airlines has the most delays and Hawaiian Airlines leaves early on average.

```{r delaysboxplot, warnings = FALSE, messages = FALSE, fig.cap = "(ref:delaysboxplot-en)", fig.width = 6}
